...
- **Hu–Sawicki f(R) Gravity:**
  - Replaces the Ricci scalar R in the Einstein-Hilbert action with a non-linear function:  
    ...
    where $m^2 \equiv H_0^2\Omega_m$. Key parameter: $f_{R0} = df/dR |_{z=0}$ and typically $n=1$.
  - Chameleon screening suppresses modifications in high-density environments.
  - Current observations constrain $|f_{R0}| < 10^{-5}$, selecting values within $[-10^{-8}, -10^{-4}]$.

- **DGP Braneworld Gravity (nDGP branch):**
  - Large-scale leakage of gravity into extra dimensions, characterized by crossover scale $r_c$.
  - Vainshtein screening suppresses fifth forces around massive objects.
  - We focus on the *ghost-free normal branch*. Constraints: $r_c \gtrsim 340$ Mpc.
...
- **Galaxy–Galaxy Auto Power Spectrum ($C_\ell^{gg}$)**
- **Galaxy–CMB Lensing Cross Power ($C_\ell^{\kappa g}$)**
...
- **Galaxy–CMB lensing cross-power spectrum:** $C_\ell^{\kappa g}$
...
- **Degeneracy and Bias Evaluation:** Assessment of parameter covariances (e.g., $f_{R0}$–$\sigma_8$). Cross-correlation specifically helps break these.
- **Emulator Integration:** MGemu (tested for Hu-Sawicki model, $k \approx 0.01$–$1$ $h$/Mpc, $|f_{R0}| = 10^{-8}$–$10^{-4}$)
...
1. **Theoretical Calculation:**
   - Compute theoretical observables ($C_\ell$) using baseline ΛCDM (CosmoPower) and MG models (MGemu, if supported for parameter range).
...
2. **Survey Modeling:**
   - **LSST/DESC Y10:**
      - Ten redshift lens bins for clustering, up to $z \sim 3$; five source bins for weak lensing
      - Effective source density $\bar{n} \sim 27.95$ arcmin$^{-2}$
      - Sky area fraction: $f_{sky} \approx 0.35$
      - Photometric redshift error: $\langle\Delta z\rangle = 0.003(1+z)$
   - **Simons Observatory LAT:**
      - Maps $\sim$40% sky with 1.4 arcmin resolution at 145 GHz
      - Total noise $\sim 6$ $\mu$K-arcmin, target for deep fields $\sim 2.8$ $\mu$K-arcmin
...
5. **Forecasting:**
   - Fisher matrix approach; careful selection of $k_{max}$, $\ell_{max}$ to avoid nonlinear and poorly-screened regimes
...
6. **Interpretation:**
   - Directly relate constraints on $f_{R0}$, $r_c$, and $\gamma$ growth index to deviations from ΛCDM and GR
...
- **MG corrections:** Scale- and redshift-dependent modifications in $P_{gg}$, $P_{mg}$, encoded via $\mu(k,z)$, $\eta(k,z)$ and screening
